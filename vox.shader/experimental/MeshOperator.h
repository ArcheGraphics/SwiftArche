...
bool contains(XBoundingBox3 aabb, simd_float3 pos) {
    return simd::all(pos > aabb.min) && simd::all(pos < aabb.max);
}

// Center of this box.
simd_float3 Center(XBoundingBox3 aabb) {
    return (aabb.min + aabb.max) * 0.5f;
}

// Converts a 3-bit index into a corner.
simd_float3 GetCorner(XBoundingBox3 aabb, uint index) {
    return (simd::float3)
    {
...
// MARK: - Sphere
float distanceToPlane(XSphere sphere, simd::float4 planeEq) {
