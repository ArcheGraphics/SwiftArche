...
// Shader representation of an XMaterial.
struct XShaderMaterial {
    texture2d <xhalf> albedo            [[id(XMaterialIndexBaseColor)]];
    texture2d <xhalf> metallicRoughness [[id(XMaterialIndexMetallicRoughness)]];
    texture2d <xhalf> normal            [[id(XMaterialIndexNormal)]];
    texture2d <xhalf> emissive          [[id(XMaterialIndexEmissive)]];
    float alpha                        [[id(XMaterialIndexAlpha)]];
    bool hasMetallicRoughness          [[id(XMaterialIndexHasMetallicRoughness)]];
    bool hasEmissive                   [[id(XMaterialIndexHasEmissive)]];

#if SUPPORT_SPARSE_TEXTURES
    uint baseColorMip           [[id(XMaterialIndexBaseColorMip)]];
    uint metallicRoughnessMip   [[id(XMaterialIndexMetallicRoughnessMip)]];
    uint normalMip              [[id(XMaterialIndexNormalMip)]];
    uint emissiveMip            [[id(XMaterialIndexEmissiveMip)]];
#endif
};
...
//  invoked from ICBs.
struct XShaderLightParams {
    constant XPointLightData *pointLightBuffer               [[id(XLightParamsIndexPointLights)]];
    constant XSpotLightData  *spotLightBuffer                [[id(XLightParamsIndexSpotLights)]];
    constant uint8_t            *pointLightIndices              [[id(XLightParamsIndexPointLightIndices)]];
    constant uint8_t            *spotLightIndices               [[id(XLightParamsIndexSpotLightIndices)]];
    constant uint8_t            *pointLightIndicesTransparent   [[id(XLightParamsIndexPointLightIndicesTransparent)]];
    constant uint8_t            *spotLightIndicesTransparent    [[id(XLightParamsIndexSpotLightIndicesTransparent)]];
};

// Structure containing textures that can be shared globally between shaders.
struct XGlobalTextures {
    depth2d<float> viewDepthPyramid    [[id(XGlobalTextureIndexViewDepthPyramid)]];

    // Forward rendering needs the following:
    depth2d_array<float> shadowMap           [[id(XGlobalTextureIndexShadowMap)]];

    texture2d <xhalf> dfgLutTex           [[id(XGlobalTextureIndexDFG)]];
    texturecube <xhalf> envMap              [[id(XGlobalTextureIndexEnvMap)]];
    texture2d<float, access::read> blueNoise   [[id(XGlobalTextureIndexBlueNoise)]];
    texture3d<float, access::read> perlinNoise [[id(XGlobalTextureIndexPerlinNoise)]];

#if USE_SCALABLE_AMBIENT_OBSCURANCE
    texture2d <xhalf> saoTexture          [[id(XGlobalTextureIndexSAO)]];
#endif

#if USE_SCATTERING_VOLUME
    texture3d <xhalf> scattering          [[id(XGlobalTextureIndexScattering)]];
#endif

#if USE_SPOT_LIGHT_SHADOWS
    depth2d_array<float> spotShadowMaps      [[id(XGlobalTextureIndexSpotShadows)]];
#endif
};
...
struct XSimpleVertexOut {
    float4 position [[position]];
};

struct XSimpleTexVertexOut {
    float4 position [[position]];
    float2 texCoord;
};
...
struct XGBufferFragOut {
    xhalf4 albedo      [[color(XGBufferAlbedoAlphaIndex)]];
    xhalf4 normals     [[color(XGBufferNormalsIndex)]];
    xhalf4 emissive    [[color(XGBufferEmissiveIndex)]];
    xhalf4 F0Roughness [[color(XGBufferF0RoughnessIndex)]];
};
...
static constant uint
HEATMAP_LEVELS = 5;

static constant float4
HEATMAP_COLORS[] =
{
float4(0,0,0,0),
float4(0,0,1,1),
...
inline float4 getHeatmapColor(uint
x,
uint num
)
{
float l = saturate((float) x / num) * HEATMAP_LEVELS;
float4 a = HEATMAP_COLORS
[(uint)
floor(l)
];
float4 b = HEATMAP_COLORS
[(uint)
ceil(l)
];
...
inline float4 worldPositionForTexcoord(float2 texCoord, float depth, constant XCameraParams &
...
worldPosition.xyz /= worldPosition.
w;
return
worldPosition;
}
...
inline float linearizeDepth(constant XCameraParams &
...
{
return
dot(float2(depth, 1), cameraParams
.invProjZ.xz) /
dot(float2(depth, 1), cameraParams
.invProjZ.yw);
}
...
inline float evaluateCascadeShadows(constant XFrameConstants &
...
if(!useFilter)
return shadowMap.
sample_compare(sam, shadowUv, cascadeIndex, lightSpaceDepth
);
...
{
shadow += shadowMap.
sample_compare(sam, shadowUv, cascadeIndex, lightSpaceDepth, int2(i, j)
);
}
}
shadow /= 9;
...
inline xhalf4 applyScattering(xhalf4 color, uint2 position, float2 texcoord, float depth,
        ...
        constant XFrameConstants &

frameData,
constant XCameraParams
& cameraParams)
{
...
dither.
x = noiseTexture.read(ditherPos0 % 64).x;
dither.
y = noiseTexture.read(ditherPos1 % 64).x;
dither.
x = goldenRatio(dither.x, frameData.frameCounter);
dither.
y = goldenRatio(dither.y, frameData.frameCounter);
dither = dither * 2.0f - 1.0f;
dither /=
float2(scattering
.

get_width(), scattering
...
{
return
applyScattering(xhalf4(color, 1.0f), position, texcoord, depth, scattering, noiseTexture, frameData, cameraParams
).
rgb;
}
