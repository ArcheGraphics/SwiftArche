...
namespace ozz {
    ...
    namespace io {
        OZZ_IO_TYPE_TAG("ozz-sample-Mesh-Part", Skin::Part)
        OZZ_IO_TYPE_VERSION(1, Skin::Part)

        template<>
        struct Extern<Skin::Part> {
            static void Save(OArchive &_archive, const Skin::Part *_parts,
                    size_t _count);

            static void Load(IArchive &_archive, Skin::Part *_parts,
                    size_t _count, uint32_t _version);
        };

        OZZ_IO_TYPE_TAG("ozz-sample-Mesh", Skin)
        OZZ_IO_TYPE_VERSION(1, Skin)

        template<>
        struct Extern<Skin> {
            static void Save(OArchive &_archive, const Skin *_meshes,
                    size_t _count);

            static void Load(IArchive &_archive, Skin *_meshes, size_t _count,
                    uint32_t _version);
        };
    }  // namespace io
}  // namespace ozz
