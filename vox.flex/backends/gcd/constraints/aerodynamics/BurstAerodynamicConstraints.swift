...
public class BurstAerodynamicConstraints: BurstConstraintsImpl<BurstAerodynamicConstraintsBatch> {
    public init(solver: BurstSolverImpl) {
        super.init(solver: solver, constraintType: Oni.ConstraintType.Aerodynamics)
    }

    override public func CreateConstraintsBatch() -> IConstraintsBatchImpl {
        let dataBatch = BurstAerodynamicConstraintsBatch(constraints: self)
    ...
    override public func RemoveBatch(batch: IConstraintsBatchImpl) {
        batches.removeAll { b in
            b === (batch as! BurstAerodynamicConstraintsBatch)
        }
