...
public class BurstVolumeConstraints: BurstConstraintsImpl<BurstVolumeConstraintsBatch> {
    public init(solver: BurstSolverImpl) {
        super.init(solver: solver, constraintType: Oni.ConstraintType.Volume)
    }

    override public func CreateConstraintsBatch() -> IConstraintsBatchImpl {
        let dataBatch = BurstVolumeConstraintsBatch(constraints: self)
    ...
    override public func RemoveBatch(batch: IConstraintsBatchImpl) {
        batches.removeAll { b in
            b === (batch as! BurstVolumeConstraintsBatch)
        }
        batch.Destroy()
    }
}
