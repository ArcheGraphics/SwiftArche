...
static xhalf3 evaluateBRDF(XPixelSurfaceData surface,
        xhalf3 viewDir,
...
static xhalf3 IBL(XPixelSurfaceData surface,
...
static xhalf3 applyPointLight(XPixelSurfaceData surface,
...
// Calculates the lighting contribution from a point light.
static xhalf3 applySpotLight(XPixelSurfaceData surface,
        XSpotLightData light,
        float localLightIntensity,
        float3 position,
        xhalf3 viewDir
#if USE_SPOT_LIGHT_SHADOWS
        , uint lightIdx, depth2d_array<float> spotShadowMaps
#endif
) {
...
static xhalf3 lightingShader(XPixelSurfaceData surfaceData,
        xhalf aoSample,
        float depth,
...
result +=
IBL(surfaceData, envMap, dfgLutTex, viewDir, frameData
.iblScale, frameData.iblSpecularScale);
